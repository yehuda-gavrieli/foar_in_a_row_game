...
def check_row(board:List[List[str]], player_color:str) -> bool:
    for row in range(ROWS):
        for col in range(COLS - 3):
            if board[row][col] == player_color and board[row][col + 1] == player_color and board[row][col + 2] == player_color and board[row][col + 3] == player_color:
                return True
def check_col(board:List[List[str]], player_color:str) -> bool:
...
def check_diagonal_left_to_right(board:List[List[str]], player_color:str) -> bool:
    for row in range(ROWS - 3):
        for col in range(COLS - 3): 
            if board[row][col] == player_color and board[row + 1][col + 1] == player_color and board[row + 2][col + 2] == player_color and board[row + 3][col + 3] == player_color:
                return True
def check_diagonal_right_to_left(board:List[List[str]], player_color:str) -> bool:
...
def check_if_win(board:List[List[str]], player_color:str) -> bool:
    if check_col(board, player_color) or check_row(board, player_color) or check_diagonal_right_to_left(board, player_color) or check_diagonal_left_to_right(board, player_color):
        return True
    return False
